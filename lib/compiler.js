...
dust.compile = function(source, name, stripSpaces) {
  try {
    if (stripSpaces === undefined) {
      stripSpaces = true;
    }

    var ast = filterAST(dust.parse(source), stripSpaces);
...
function filterAST(ast, stripSpaces) {
  var context = {
...
function stripSpaces(context, node) {
  if (context.stripSpaces) {
    return nullify();
  }
  return node;
}
