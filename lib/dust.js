...
dust.escapeHref = (function() {
	...
	if (encodeURIComponent('!\'()*') === '!\'()*') {
		...
		encodeURIComponent = function() {
			...
			return _encodeURIComponent.apply(this, arguments).replace(/[!'()]/g, escape).replace(/\*/g, "%2A");
		};
	}
	...
	var sanitizeURL = function(url) {
		url = url.trim()
			...
			.replace(controlCharacters, "\uFFFD") // replace control characters
			.replace(new RegExp("\\t", "g"), "%09")
			.replace(new RegExp("\\r", "g"), "%0D")
			.replace(new RegExp("\\n", "g"), "%0A")
			.replace(new RegExp("[\\s\\u2028\\u2029]", "g"), "%20")	// all custom spaces must be URI encoded
			.replace(new RegExp("&#", "g"), "#")	// http://w3.org/test?a=b&#x3c;
			.replace(new RegExp("(&[a-zA-Z]+);", "g"), "$1%3B")	// javascript&NewLine;&Tab;&colon;&apos; but not &quot&lt&gt&amp
			.replace(new RegExp("^([\\s\\S]{0,14});", "g"), "$1%3B")	// https://;URL=javascript:alert(1)
			.replace(new RegExp("(&[quolgQUOLG]{1,3})t", "g"), "$1%74")	// &qUot, &lt, &Gt
			.replace(new RegExp("(&[quolgQUOLG]{1,3})T", "g"), "$1%54")	// &quoT, &LT, &gT
			.replace(new RegExp("`", "g"), "%60")
			.replace(new RegExp("'", "g"), "%27")
			.replace(new RegExp("\"", "g"), "%22")
			.replace(new RegExp("<", "g"), "%3C")
			.replace(new RegExp(">", "g"), "%3E")
			.replace(new RegExp("\\(", "g"), "%28")
			.replace(new RegExp("\\)", "g"), "%29")
			.replace(new RegExp("\\[", "g"), "%5B")
			.replace(new RegExp("\\]", "g"), "%5D")
			.replace(new RegExp("\\{", "g"), "%7B")
			.replace(new RegExp("\\}", "g"), "%7D")
			.replace(new RegExp("\\\\", "g"), "%5C")
			.replace(new RegExp(",", "g"), "%2C")
			.replace(new RegExp("%(?=[^A-Fa-f0-9]|[A-Fa-f0-9][^A-Fa-f0-9]|[\\s\\S]?$)", "g"), "%25");

		var lc = url.toLowerCase();
		if (!(new RegExp("^(?:http|mailto|ftp|[#/.?])")).test(lc) && (new RegExp("^[^?/#:]+?:")).test(lc)) {
			if ((new RegExp("^(?:about|cdl|dvd|local|mhtml|mk|ms-help|ms-its|tv|res|its|javascript|data|vbs|vbscript|feed|java|jar|file|[a-z]):")).test(lc) || !(new RegExp("^[a-z0-1\\-_.]+?:")).test(lc)) {
				url = "./" + url;
			}
		}
...
dust.escapeHtml = (function(undefined) {
...
dust.escapeJs = (function(undefined) {
	...
	// check that JSON.stringify is native and override it
	if (typeof(JSON) !== 'undefined' && JSON.stringify && JSON._stringify === undefined && JSON.stringify('\\"\/<>&%\u0000\u2028\u2029*()\'=!?`#') === '"\\\\\\"/<>&%\\u0000\u2028\u2029*()\'=!?`#"') {
		JSON._stringify = JSON.stringify;
		JSON.stringify = function() {
			return JSON._stringify.apply(this, arguments).replace(jsonRE, function(c) {
				return index[c];
			});
		};
	}
	
	return jsEncode = function(s) {
		if (s === null || s === undefined) {
			return null;
		}
