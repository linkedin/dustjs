(function (root, factory) {
  ...
  Context.prototype._get = function(cur, down) {
    ...
    function continueSearchingWithAdditionalData(data) {
      return currentContext.push(data)._get(cur, down.slice(i));
    }
    ...
    if (cur && len === 0) {
      ...
      while (ctx && i < len) {
        if (dust.isThenable(ctx)) {
          ...
          return ctx.then(continueSearchingWithAdditionalData);
        }
        ctxThis = ctx;
        ctx = ctx[down[i]];
        i++;
