...
(function(root) {
  ...
  function DustCustomError(message) {
    this.name = "DustCustomError";
    this.message = (message || "");
  }
  DustCustomError.prototype = new Error();
  DustCustomError.prototype.constructor = DustCustomError;

  dust.customError = DustCustomError;
  /**
  ...
  dust.onError = function(error, chunk) {
    ...
    if(dust.isDebug || (!dust.silenceAllErrors && !(error instanceof dust.customError))) {
      throw error;
    } else {
      return chunk;
    }
    ...
    return function(context, callback) {
      ...
      dust.nextTick(function() {
        if(typeof tmpl === 'function') {
          ...
          dust.onError(new dust.customError('Template [' + name + '] cannot be resolved to a Dust function'));
        }
      });
      return master;
    };
  ...
  dust.load = function(name, chunk, context) {
    ...
    if (tmpl) {
      ...
      return chunk.setError(new dust.customError('Template Not Found: ' + name));
    }
  };
    ...
    if (filters) {
      for (var i=0, len=filters.length; i<len; i++) {
        ...
        if (name === 's') {
          ...
          dust.onError(new dust.customError('Invalid filter [' + name + ']'));
        }
      }
    }
  ...
  Stub.prototype.flush = function() {
    ...
    while (chunk) {
      if (chunk.flushable) {
        ...
        dust.onError(new dust.customError('Chunk error [' + chunk.error + '] thrown. Ceasing to render this template.'));
  ...
  Stream.prototype.emit = function(type, data) {
    ...
    if (typeof handler === 'function') {
      ...
      dust.onError(new dust.customError('Event Handler [' + handler + '] is not of a type that is handled by emit'));
    }
  };
  ...
  Chunk.prototype.helper = function(name, context, bodies, params) {
    ...
    try {
      if(dust.helpers[name]) {
        ...
        return dust.onError(new dust.customError('Invalid helper [' + name + ']'), chunk);
      }
    } catch (err) {
