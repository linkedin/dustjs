    ...
    if (cur && len === 0) {
      ...
      if (!cur) {
        ...
        while (ctx) {
          if (ctx.isObject) {
            ctxThis = ctx.head;
            value = hasOwnProperty.call(ctx.head, first) ? ctx.head[first] : undefined;
            if (value !== undefined) {
              break;
            }
          }
        ...
        if (value !== undefined) {
          ...
          ctx = (this.global && hasOwnProperty.call(this.global, first)) ? this.global[first] : undefined;
        }
      } else if (ctx) {
        // if scope is limited by a leading dot, don't search up the tree
        if(ctx.head && hasOwnProperty.call(ctx.head, first)) {
          ctx = ctx.head[first];
        } else {
      ...
      while (ctx && i < len) {
        ctxThis = ctx;
        ctx = hasOwnProperty.call(ctx, down[i]) ? ctx[down[i]] : undefined;
        i++;
      }
    }
