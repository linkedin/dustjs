...
dust.filters = {
  ...
  size: function(value) {
    if (!value) { //undefined, "", 0
        return 0;
    } else if(dust.isArray(value)) { //array
        return value.length;
    } else if (!isNaN(value)) { //numeric values
        return value;
    } else if (Object(value) === value) { //object test
        var nr = 0;
        for(var k in value) if(Object.hasOwnProperty.call(value,k)) nr++;

        return nr;
    } else {
        return (value + '').length; //any other value (strings etc.)
    }
  }
};
...
Chunk.prototype.reference = function(elem, context, auto, filters) {
  ...
  var filterValue = dust.filter(elem, auto, filters);

  if (!dust.isEmpty(filterValue)) {
    return this.write(filterValue);
  } else {
    return this;
  }
};
