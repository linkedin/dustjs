...
(function (root, factory) {
  if (typeof exports === 'object') {
    ...
    /* 
     * for the browser we depend on dust being defined in the global namespace 
     * attach the parser the the global dust object
     */
    root.dust.parse = factory().parse;
  }
}(this, function () {
  /*
   * @returns a dust parser
   */
var parser = (function(){
  /*
...
(function (root, factory) {
  if (typeof exports === 'object') {
     ...
     * attach the parser the the global dust object
     */
    root.dust.compile = factory(dust).compile;
  }
}(this, function (parser) {
  /*
   * @returns compiler - the dust compiler
   * @requires parser - the dust parser
   */
...
var dustCompile = function(source, name, strip) {
  try {
    if (strip) {
      source = source.replace(/^\s+/mg, '').replace(/\n/mg, '');
    }
    var ast = filterAST(parser.parse(source));
...
function filterAST(ast) {
  var context = {};
  return filterNode(context, ast);
}

function filterNode(context, node) {
  return optimizers[node[0]](context, node);
}
...
function visit(context, node) {
  ...
  for (var i=1, len=node.length; i<len; i++) {
    var res = filterNode(context, node[i]);
    if (res) out.push(res);
  }
  return out;
}
...
nodes = {
  ...
  reference: function(context, node) {
    return ".reference(" + compileNode(context, node[1])
  ...
  "%": function(context, node) {
    ...
    return pragmas[name](context, ctx, bodies, params);
  },
...
var escape = (typeof JSON === "undefined")
  ? function(str) { return "\"" + escapeJs(str) + "\"" }
  : JSON.stringify;
